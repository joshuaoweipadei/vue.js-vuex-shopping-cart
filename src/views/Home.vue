      ...
      <div v-for="product in products" :key="product.id" class="col-md-4 col-lg-3">
        <div class="card my-2">
          ...
          <div class="card-body">
            ...
            <button :disabled="inCart.includes(product.id)" @click="addCartItem(product.id)" class="btn btn-success btn-sm"><i class="fa fa-shopping-cart"></i> {{inCart.includes(product.id) === true ? 'Added' : 'Add To Cart'}}</button>
          </div>
        </div>
      </div>
